...
/**
 * Национальная часть: 9 цифр после +375 / 375 / 80.
 * @param {string} nine
 */
const isValidBelarusNational = (nine) => nine.length === 9 && /^[1-9]\d{8}$/.test(nine);

/**
 * Беларусь: +375, 375 или 80 (8 0 …), затем 9 цифр.
 * @param {string} phone
 * @returns {string|null}
 */
const validateBelarusPhone = (phone) => {
...
/**
 * Городской телефон РБ: +375 17 … (Минск), +375 152 … и др.
 * @param {string} phone
 * @returns {string|null}
 */
const validateBelarusLandline = (phone) => {
  const trimmed = String(phone || "").trim();
  if (!trimmed) {
    return null;
  }
...
/**
 * Общий телефон: мобильный или городской.
 * @param {string} phone
 * @returns {string|null}
 */
const validateBelarusAnyPhone = (phone) => {
  return validateBelarusPhone(phone) || validateBelarusLandline(phone);
};
...
const { CLIENT_CONTACT_POINT_TYPES } = require("../config/clientContactPointTypes");
 ...
 * @returns {string|null}
 */
const validateClientContactPointValue = (type, value) => {
  ...
  switch (normalizedType) {
    ...
    default:
      if (!CLIENT_CONTACT_POINT_TYPES.includes(normalizedType)) {
        return `Недопустимый тип контактной точки: ${type || "(пусто)"}.`;
      }
      return null;
  }
};
